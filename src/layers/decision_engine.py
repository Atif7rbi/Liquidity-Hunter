...
v1.4 changes:
- Added Funding Bias soft penalty from ContextResult.funding_bias:
    🟡 conflict  → score × 0.85
    🟡 aligned   → score × 1.05
"""
...
class DecisionResult:
    ...
    veto_reason: str = ""          # filled when a veto fires
...
class DecisionEngine:
    ...
    def evaluate(
        ...
        # 🟦 Context Veto — soft (score multiplier only)
        ...
        # ── 🟡 Funding Bias — soft penalty (v1.4) ──────────────────
        # conflict → score × 0.85 | aligned → score × 1.05
        if ctx.funding_bias != "NEUTRAL":
            if ctx.funding_bias != direction:
                score *= 0.85
                reasoning.append(
                    f"Funding bias conflict: ctx={ctx.funding_bias} vs signal={direction} → score ×0.85"
                )
            else:
                score = min(score * 1.05, 100.0)
                reasoning.append(
                    f"Funding bias aligned: ctx={ctx.funding_bias} with signal={direction} → score ×1.05"
                )
