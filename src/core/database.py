...
v1.3 changes:
- Trade: added mae_pct, mfe_pct (tracked during position lifetime)
- TradeOutcome: added mae_r, mfe_r, time_in_trade_min, time_to_mfe_min,
                sl_price, tp_price, veto_reason
  All new columns are nullable=True — no migration needed for SQLite.
"""
...
class Trade(Base):
    ...
    # ── v1.3: MAE/MFE tracking (updated on each tick by paper_executor) ──
    mae_pct:    Mapped[Optional[float]]    = mapped_column(Float, nullable=True)   # max adverse  %
    mfe_pct:    Mapped[Optional[float]]    = mapped_column(Float, nullable=True)   # max favorable %
...
class TradeOutcome(Base):
    ...
    # ── v1.3: MAE / MFE analytics ────────────────────────────────────────
    mae_r:             Mapped[Optional[float]] = mapped_column(Float, nullable=True)    # max adverse in R
    mfe_r:             Mapped[Optional[float]] = mapped_column(Float, nullable=True)    # max favorable in R
    time_in_trade_min: Mapped[Optional[int]]   = mapped_column(Integer, nullable=True)  # total duration
    time_to_mfe_min:   Mapped[Optional[int]]   = mapped_column(Integer, nullable=True)  # minutes to peak
    sl_price:          Mapped[Optional[float]] = mapped_column(Float, nullable=True)    # SL at entry
    tp_price:          Mapped[Optional[float]] = mapped_column(Float, nullable=True)    # TP2 at entry
